...
```{r}
# note that prs_pairs uses different opengwasid
t2d_top <- subset(dat, id.exposure == "ebi-a-GCST005047" & id.outcome %in% unique(subset(prs_pairs, opengwasid=="ieu-a-26"))$prot[1:56])
...
```{r}
ms_top <- subset(dat, id.exposure == "ieu-a-1024" & id.outcome %in% unique(subset(prs_pairs, opengwasid=="ieu-b-18"))$prot[1:56])
